...
await fs.writeFile(
  ...
  return Response.json({ ok: true, repo: env.W7S_REPOSITORY, path: new URL(request.url).pathname });
}
`
);
...
try {
  ...
  const backendBody = (await backendResponse.json()) as { ok: boolean; repo: string; path: string };
